...
Una vez que se tiene instalado y configurado Git, y una cuenta en Github, basta con hacer un \emph{push} desde el repositorio local hacia el servidor. Por ejemplo, para copiar este documento hacia el servidor que lo aloja, se utilizó el comando:
\begin{verbatim}
git push
\end{verbatim}
